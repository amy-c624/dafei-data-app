...
if check_password():
    ...
    if uploaded_file:
        ...
        # --- 置頂：人工品名對位區 (含隱藏機制) ---
        unique_specs = sorted(processed['原始規格'].unique())
        manual_mapping = {}

        if not st.session_state.get('spec_confirmed', False):
            with st.expander("🔍 品名規格自動分類檢查 (請確認或修正)", expanded=True):
                st.info("下方為系統根據邏輯預先分類的結果。若有誤請修正，確認無誤請按下方按鈕。")
                for spec in unique_specs:
                    # 抓取該品名目前的自動分類
                    default_att = processed[processed['原始規格'] == spec]['人次分類'].iloc[0]
                    default_rev = processed[processed['原始規格'] == spec]['營收分類'].iloc[0]
                    
                    c1, c2, c3, c4 = st.columns([2, 1.2, 1.2, 1.2])
                    with c1: st.write(f"**{spec}**")
                    with c2: 
                        opt_att = ["不計人次", "VIP", "VVIP", "平台", "股東票", "校園票", "散客", "無視", "團購券", "團體", "親子卡", "電競館"]
                        m_att = st.selectbox("人次", opt_att, index=opt_att.index(default_att) if default_att in opt_att else 0, key=f"m_att_{spec}")
                    with c3:
                        opt_rev = ["周邊商品", "檔期商品", "票務", "平台", "無視", "預售票", "電競館收入", "平台收入"]
                        m_rev = st.selectbox("營收", opt_rev, index=opt_rev.index(default_rev) if default_rev in opt_rev else 0, key=f"m_rev_{spec}")
                    with c4:
                        final_rev = m_rev
                        if m_rev == "檔期商品":
                            m_key = st.text_input("檔期名", placeholder="如：巨人", key=f"m_key_{spec}")
                            final_rev = f"{m_key}周邊商品" if m_key else "檔期周邊商品"
                        else: st.write("---")
                    manual_mapping[spec] = {"att": m_att, "rev": final_rev}
                
                if st.button("✅ 確認品名分類無誤 (隱藏此區塊)"):
                    st.session_state.spec_confirmed = True
                    st.rerun()
        ...
        if final_list:
            full_film_df = pd.concat(final_list).reset_index(drop=True)
        ...
        for s_name, s_qty in slots:
            for h_type in sel_hols:
                ...
                if d_num == 0: continue
                act_qty = s_qty
                if sel_site == "i-Ride TAIPEI" and s_name == "20:00-21:00": act_qty = 5 if h_type == "假日" else 4
                ...
                occ_list.append({'時段': s_name, '類型': h_type, '值': rate, '稼動率': f"{rate:.2f}%"})

        if occ_list:
            occ_df = pd.DataFrame(occ_list)
            p_df = occ_df.pivot(index='時段', columns='類型', values='稼動率').fillna("-")
            avg_row = {h: f"平均: {occ_df[occ_df['類型']==h]['值'].mean():.2f}%" for h in sel_hols if h in p_df.columns}
            final_occ = pd.concat([p_df, pd.DataFrame(avg_row, index=['平均稼動率'])])
