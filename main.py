...
if check_password():
    # 1. 假期定義 (根據 DGPA 最新 2025-2026 辦公日曆表更新)
    def get_holiday_type(date):
        if pd.isna(date): return "未知"
        d_str = date.strftime('%Y-%m-%d')
        
        # 2025-2026 國定假日與補假清單 (包含新增之教師節、光復節、行憲紀念日等)
        holidays_2025 = [
            '2025-01-01', # 元旦
            '2025-01-25', '2025-01-26', '2025-01-27', '2025-01-28', '2025-01-29', '2025-01-30', '2025-01-31', '2025-02-01', '2025-02-02', # 春節連假 (1/25-2/2)
            '2025-02-28', '2025-03-01', '2025-03-02', # 228連假
            '2025-04-03', '2025-04-04', '2025-04-05', '2025-04-06', # 清明連假 (4/3-4/6)
            '2025-05-01', # 勞動節 (全台放假)
            '2025-05-30', '2025-05-31', '2025-06-01', # 端午連假 (5/30補假-6/1)
            '2025-09-27', '2025-09-28', '2025-09-29', # 教師節連假 (9/28週日, 9/29補假)
            '2025-10-04', '2025-10-05', '2025-10-06', # 中秋連假 (10/6週一放假)
            '2025-10-10', '2025-10-11', '2025-10-12', # 國慶連假
            '2025-10-24', '2025-10-25', '2025-10-26', # 光復節連假 (10/25週六, 10/24補假)
            '2025-12-25'  # 行憲紀念日
        ]
        
        holidays_2026 = [
            '2026-01-01', # 元旦
            '2026-02-14', '2026-02-15', '2026-02-16', '2026-02-17', '2026-02-18', '2026-02-19', '2026-02-20', '2026-02-21', '2026-02-22', # 春節連假 (2/14-2/22)
            '2026-02-27', '2026-02-28', '2026-03-01', # 228連假 (2/28週六, 2/27補假)
            '2026-04-03', '2026-04-04', '2026-04-05', '2026-04-06', # 清明連假 (4/4-4/5遇週末, 4/3及4/6補假)
            '2026-05-01', '2026-05-02', '2026-05-03', # 勞動節連假
            '2026-06-19', '2026-06-20', '2026-06-21', # 端午連假
            '2026-09-25', '2026-09-26', '2026-09-27', '2026-09-28', # 中秋+教師節連假 (9/25中秋, 9/28教師節)
            '2026-10-09', '2026-10-10', '2026-10-11', # 國慶連假 (10/10週六, 10/9補假)
            '2026-10-24', '2026-10-25', '2026-10-26', # 光復節連假 (10/25週日, 10/26補假)
            '2026-12-25', '2026-12-26', '2026-12-27'  # 行憲紀念日連假
        ]
        
        all_holidays = holidays_2025 + holidays_2026
        
        # 判定規則：如果在清單中，或本身是星期六、日，即為假日
    ...
    def process_data(df):
        ...
        df['統計用營收'] = df.apply(lambda x: 0 if x['營收分類'] == '無視' else x['含稅營營收'] if '含稅營營收' in x else x['含稅營收'], axis=1)
        return df
