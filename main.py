...
def check_password():
    if "password_correct" not in st.session_state:
        def password_entered():
            if st.session_state["password"] == "TEST": # 密碼維持 TEST
...
st.set_page_config(page_title="i-Ride 營運決策系統 (全功能整合版)", layout="wide")
...
if check_password():
    ...
    if uploaded_file:
        ...
        def classify(row):
            ...
            # A. 優先規則判定 (排除手動判定干擾)
            # 1. 校園票捕捉 (最高優先)
            if any(x in spec for x in ['校園', '學生']) or cid == 'Z00054' or "校園" in pname:
                if "商品" not in spec or "票" in spec: res_att_cat = "校園票"
            
            # 2. 忽略項 (如截圖中的券差額、郵票、贈品)
            elif any(x in spec for x in ['券差額', '溢收', '商品兌換', '核銷', '免費', '員工', '郵票', '貼紙']):
                res_att_cat = "無視"
            
            # 3. 企業與團體 (含截圖中的企業優惠票)
            elif any(x in spec for x in ['企業', '團體', '優惠票', '套票', '通路']):
                res_att_cat = "團體"

            # 4. 股東/VIP/電競
            elif '股東' in spec: res_att_cat = "股東票"
            ...
            elif any(k in spec for k in ['VR','體感','賽車','僵屍','LED']): res_att_cat = "電競館"
            elif any(x in spec for x in ['市民', '愛心', '成人', '現場']): res_att_cat = "散客"

            # B. 衝突攔截 (只有當上面都沒對到，且有影片或是明顯是票時才跳出)
            if res_att_cat == "待確認票種":
                if has_video or any(x in spec for x in ['票','券','卡']):
                    is_anomaly = True

            # C. 數值指派
            if res_att_cat not in ["無視", "待確認票種"]:
                res_watch_val = qty
            ...
            if res_att_cat == "無視" or spec in ['商品兌換券', '票券核銷']: res_rev = "無視"
        ...
        if len(anomaly_specs) > 0 and not st.session_state.anomaly_finished:
            with st.form("anomaly_fix_form"):
                st.error(f"🚨 偵測到 {len(anomaly_specs)} 項無法自動辨識之規格：")
                temp_fixes = {}
                for spec in anomaly_specs:
                    c1, c2, c3, c4 = st.columns([2, 1, 1, 1])
                    with c1: st.write(f"**{spec}**")
                    with c2: temp_fixes[f"{spec}_att"] = st.selectbox("人次類別", ["散客", "校園票", "平台", "VIP", "股東票", "團體", "親子卡", "無視"], key=f"a_att_{spec}")
        ...
        if st.session_state.anomaly_finished or len(anomaly_specs) == 0:
            ...
            with t2:
                st.subheader("👥 人次分類小計")
                st.table(f_df_valid.groupby('人次分類')[['計算人次', '觀看總數']].sum().reset_index().style.format({'計算人次': '{:,.0f}', '觀看總數': '{:,.0f}'}))

            st.divider()
            st.subheader("🎬 影片觀看分類分析")
